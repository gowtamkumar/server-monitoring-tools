...
/**
 * Get monthly bandwidth breakdown from vnstat (last 12 months)
 */
async function getMonthlyBandwidth(ssh) {
  try {
    const result = await execCommand(ssh, "vnstat --json");
    if (!result.stdout) return { error: 'vnstat not installed or no data', months: [] };

    const data = JSON.parse(result.stdout);
    const iface = data.interfaces?.[0];
    if (!iface) return { error: 'No interface data', months: [] };

    const months = (iface.traffic?.month || [])
      .slice(-12) // last 12 months
      .map(m => ({
        month: `${m.date?.year}-${String(m.date?.month).padStart(2, '0')}`,
        label: new Date(m.date?.year, (m.date?.month || 1) - 1, 1)
          .toLocaleString('default', { month: 'short', year: 'numeric' }),
        rx: m.rx || 0,
        tx: m.tx || 0,
        rx_formatted: formatBytes(m.rx || 0),
        tx_formatted: formatBytes(m.tx || 0),
        total: (m.rx || 0) + (m.tx || 0),
        total_formatted: formatBytes((m.rx || 0) + (m.tx || 0)),
      }));
    ...
    return { error: err.message, months: [] };
  }
}
